...
class KMeans {
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    for (m <- oldMeans) yield (findAverage(m, classified(m)))
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    // def _converged(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans zip newMeans map {case (x1, x2) => x1 squareDistance x2} forall (_ <= eta)
    // }
    // _converged(oldMeans, newMeans)
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    ...
    def convergence = converged(eta) _
    if (!convergence(means, newMeans))
