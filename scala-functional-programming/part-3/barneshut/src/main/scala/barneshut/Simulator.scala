...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    ...
    newBoundaries.minX = min(body.x, boundaries.minX)
    newBoundaries.minY = min(body.y, boundaries.minY)
    newBoundaries.maxX = max(body.x, boundaries.maxX)
    newBoundaries.maxY = max(body.y, boundaries.maxY)
    newBoundaries
  }

  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val newBoundaries = new Boundaries
    newBoundaries.minX = min(a.minX, b.minX)
    newBoundaries.minY = min(a.minY, b.minY)
    newBoundaries.maxX = max(a.maxX, b.maxX)
    newBoundaries.maxY = max(a.maxY, b.maxY)
    newBoundaries
  }
