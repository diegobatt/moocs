...
import FloatOps._
  ...
  test("Fork with 4 empty quadrants") {
    val quad = Fork(
        Empty(10f, 15f, 1f), Empty(20f, 25f, 1f), Empty(30f, 35f, 1f), Empty(40f, 45f, 1f)
    )
    assert(quad.massX == 15f, s"should be 15f")
  }

  // test("Leaf.insert(b) should return a new Fork if size > minimumSize") {
  //   var quad: Quad = Empty(0.000001f, 1f, 1f)
  //   val b1 = new Body(100f, 2f, 2f, 0f, 0f)
  //   val b2 = new Body(100f, 4f, 4f, 0f, 0f)
  //   quad = quad.insert(b1)
  //   println("Hoja", quad)
  //   quad = quad.insert(b2)
  //   println("Final", quad)
