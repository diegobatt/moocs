...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeedsPre = List("t01", "t03", "t11", "t1801", "t1803")
    val workingActivitiesPre = List("t05", "t1805")
    val otherActivitiesPre = List(
      "t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18"
    )
    def colsFilt(prefixes: List[String]): List[Column] = {
      columnNames.filter(s => prefixes.exists(s.startsWith(_)))
                 .map(column(_))
    }
    (colsFilt(primaryNeedsPre), colsFilt(workingActivitiesPre), colsFilt(otherActivitiesPre))
  }
  ...
  def timeUsageSummary(
    ...
    val primaryNeedsProjection: Column = primaryNeedsColumns.reduce(_ + _).divide(lit(60)).as("primaryNeeds")
    val workProjection: Column = workColumns.reduce(_ + _).divide(lit(60)).as("work")
    val otherProjection: Column = otherColumns.reduce(_ + _).divide(lit(60)).as("other")
    df
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(r => (r.working, r.sex, r.age))
          ...
          .map{
            case ((working, sex, age), primary, work, other)
            => TimeUsageRow(
              working.asInstanceOf[String],
              sex.asInstanceOf[String],
              age.asInstanceOf[String],
              primary.asInstanceOf[Double],
              work.asInstanceOf[Double],
