...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd
    .map(_.mentionsLanguage(lang))
    .aggregate(0)(
      (acc: Int, x: Boolean) => acc + (if (x) 1 else 0),
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    val ranking: List[(String, Int)] = langs
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    def langsArticle(article: WikipediaArticle): List[(String, WikipediaArticle)] = {
      langs.filter(article.mentionsLanguage).map(l => (l, article))
    }
    rdd.flatMap(langsArticle).groupByKey()
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    def langsArticle(article: WikipediaArticle): List[(String, Int)] = {
      langs.filter(article.mentionsLanguage).map(l => (l, 1))
    }
    rdd
    .flatMap(langsArticle)
    .reduceByKey(_ + _)
    .sortBy(-_._2)
    .collect()
    .toList
